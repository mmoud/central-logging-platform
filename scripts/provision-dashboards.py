...
def pmg_dashboard() -> dict:
    ...
    overview = [
        ...
        ("Recent Mail Events", "table", f"SELECT _timestamp AS event_time, mail_filter_id AS filter_id, mail_message_id AS message_id, mail_queue_id AS queue_id, syslog_program AS component, mail_header_from AS header_sender, mail_envelope_sender AS envelope_sender, mail_header_to AS header_recipient, mail_envelope_recipients AS envelope_recipients, mail_subject AS subject, mail_rule AS rule, mail_filter_action AS filter_action, mail_status AS status, message FROM {s} ORDER BY _timestamp DESC LIMIT 200", [("Time", "event_time"), ("PMG Filter ID", "filter_id"), ("Message-ID", "message_id"), ("Queue ID", "queue_id"), ("Component", "component"), ("Original / Header Sender", "header_sender"), ("Envelope Sender", "envelope_sender"), ("Header Recipient", "header_recipient"), ("Envelope Recipient", "envelope_recipients"), ("Subject", "subject"), ("Rule", "rule"), ("Filter Action", "filter_action"), ("Status", "status"), ("Message", "message")], []),
    ]
    flow = [
        ("Top Original / Header Senders", "bar", f"SELECT mail_header_from AS label, count(DISTINCT mail_filter_id) AS value FROM {s} WHERE mail_header_from IS NOT NULL AND mail_header_from <> '' GROUP BY label ORDER BY value DESC LIMIT 20", [("Original / Header Sender", "label")], [("Messages", "value")]),
