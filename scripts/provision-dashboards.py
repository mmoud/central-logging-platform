...
def pmg_dashboards() -> dict[str, dict]:
    ...
    overview = [
        ...
        ("Message Volume by Day", "line", f"SELECT histogram(_timestamp, '1 day') AS ts, count(DISTINCT mail_filter_id) AS value FROM {s} WHERE mail_filter_id IS NOT NULL GROUP BY ts ORDER BY ts", [("Day", "ts")], [("Messages", "value")]),
        ...
        ("Postfix / PMG Components", "h-bar", f"SELECT syslog_program AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC LIMIT 20", [("Component", "label")], [("Events", "value")]),
    ...
    flow = [
        ...
        ("Top Header Sender Domains", "h-bar", f"SELECT mail_header_sender_domain AS label, count(DISTINCT mail_filter_id) AS value FROM {s} WHERE mail_header_sender_domain IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Domain", "label")], [("Messages", "value")]),
        ...
        ("Top Envelope Sender Domains", "h-bar", f"SELECT mail_envelope_sender_domain AS label, count(DISTINCT mail_filter_id) AS value FROM {s} WHERE mail_envelope_sender_domain IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Domain", "label")], [("Messages", "value")]),
    ...
    filtering = [
        ...
        ("Deferred by Relay", "h-bar", f"SELECT coalesce(mail_relay,'unknown') AS label, count(*) AS value FROM {s} WHERE lower(mail_status) = 'deferred' GROUP BY label ORDER BY value DESC LIMIT 20", [("Relay", "label")], [("Deferred", "value")]),
        ("DSN Distribution", "h-bar", f"SELECT mail_dsn AS label, count(*) AS value FROM {s} WHERE mail_dsn IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("DSN", "label")], [("Events", "value")]),
...
def fortigate_dashboard() -> dict:
    ...
    overview = [
        ...
        ("Actions", "h-bar", f"SELECT coalesce(fortigate_action,'none') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC LIMIT 20", [("Action", "label")], [("Events", "value")]),
    ...
    traffic = [
        ("Top Source IPs", "h-bar", f"SELECT fortigate_srcip AS label, count(*) AS value FROM {s} WHERE fortigate_srcip IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Source IP", "label")], [("Sessions", "value")]),
        ("Top Destination IPs", "h-bar", f"SELECT fortigate_dstip AS label, count(*) AS value FROM {s} WHERE fortigate_dstip IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Destination IP", "label")], [("Sessions", "value")]),
        ("Top Policies", "h-bar", f"SELECT coalesce(fortigate_policyname,fortigate_policyid) AS label, count(*) AS value FROM {s} WHERE fortigate_policyid IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Policy", "label")], [("Sessions", "value")]),
        ("Top Services", "h-bar", f"SELECT fortigate_service AS label, count(*) AS value FROM {s} WHERE fortigate_service IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Service", "label")], [("Sessions", "value")]),
        ("Applications", "h-bar", f"SELECT coalesce(fortigate_app,fortigate_appcat) AS label, count(*) AS value FROM {s} WHERE fortigate_app IS NOT NULL OR fortigate_appcat IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Application", "label")], [("Sessions", "value")]),
        ("Source Countries", "h-bar", f"SELECT fortigate_srccountry AS label, count(*) AS value FROM {s} WHERE fortigate_srccountry IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Country", "label")], [("Sessions", "value")]),
        ("Destination Countries", "h-bar", f"SELECT fortigate_dstcountry AS label, count(*) AS value FROM {s} WHERE fortigate_dstcountry IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Country", "label")], [("Sessions", "value")]),
    ...
    security = [
        ("Attack Signatures", "h-bar", f"SELECT fortigate_attack AS label, count(*) AS value FROM {s} WHERE fortigate_attack IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Attack", "label")], [("Events", "value")]),
        ...
        ("Blocked by Policy", "h-bar", f"SELECT coalesce(fortigate_policyname,fortigate_policyid,'unknown') AS label, count(*) AS value FROM {s} WHERE lower(coalesce(fortigate_action,'')) IN ('deny','blocked','block','dropped') GROUP BY label ORDER BY value DESC LIMIT 20", [("Policy", "label")], [("Blocks", "value")]),
        ("Threat Sources", "h-bar", f"SELECT fortigate_srcip AS label, count(*) AS value FROM {s} WHERE fortigate_attack IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Source IP", "label")], [("Threats", "value")]),
        ("Threat Destinations", "h-bar", f"SELECT fortigate_dstip AS label, count(*) AS value FROM {s} WHERE fortigate_attack IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Destination IP", "label")], [("Threats", "value")]),
    ...
    system = [
        ...
        ("Admin / Authentication Users", "h-bar", f"SELECT coalesce(fortigate_user,'unknown') AS label, count(*) AS value FROM {s} WHERE fortigate_user IS NOT NULL OR lower(coalesce(fortigate_subtype,'')) LIKE '%admin%' OR lower(message) LIKE '%login%' GROUP BY label ORDER BY value DESC LIMIT 20", [("User", "label")], [("Events", "value")]),
    ...
    app_control = [
        ("Top Applications", "h-bar", f"SELECT fortigate_app AS label, count(*) AS value FROM {s} WHERE fortigate_app IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("Application", "label")], [("Events", "value")]),
        ("Application Categories", "h-bar", f"SELECT fortigate_appcat AS label, count(*) AS value FROM {s} WHERE fortigate_appcat IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("Category", "label")], [("Events", "value")]),
        ...
        ("Application Control Profiles", "h-bar", f"SELECT coalesce(fortigate_applist,'unknown') AS label, count(*) AS value FROM {s} WHERE fortigate_app IS NOT NULL OR fortigate_applist IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Profile", "label")], [("Events", "value")]),
    ...
    ips = [
        ("Top IPS Signatures", "h-bar", f"SELECT fortigate_attack AS label, count(*) AS value FROM {s} WHERE fortigate_attack IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("Signature", "label")], [("Events", "value")]),
        ("Top Attack IDs", "h-bar", f"SELECT fortigate_attackid AS label, count(*) AS value FROM {s} WHERE fortigate_attackid IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("Attack ID", "label")], [("Events", "value")]),
    ...
    web_dns = [
        ("Top Requested Hosts", "h-bar", f"SELECT fortigate_hostname AS label, count(*) AS value FROM {s} WHERE fortigate_hostname IS NOT NULL AND lower(coalesce(fortigate_subtype,'')) IN ('webfilter','dns','app-ctrl') GROUP BY label ORDER BY value DESC LIMIT 25", [("Host", "label")], [("Requests", "value")]),
        ("Top URLs", "h-bar", f"SELECT fortigate_url AS label, count(*) AS value FROM {s} WHERE fortigate_url IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("URL", "label")], [("Requests", "value")]),
...
def juniper_dashboard() -> dict:
    ...
    trends = [
        ...
        ("Top Processes", "h-bar", f"SELECT coalesce(syslog_program,'unknown') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC LIMIT 20", [("Process", "label")], [("Events", "value")]),
        ("Events by Device", "h-bar", f"SELECT coalesce(device_name,host_name,source_ip,'unknown') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC LIMIT 20", [("Device", "label")], [("Events", "value")]),
    ]
    interfaces = [
        ("Top Interfaces", "h-bar", f"SELECT juniper_interface AS label, count(*) AS value FROM {s} WHERE juniper_interface IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("Interface", "label")], [("Events", "value")]),
    ...
    routing = [
        ...
        ("Neighbor / Peer Events", "h-bar", f"SELECT juniper_peer_ip AS label, count(*) AS value FROM {s} WHERE juniper_peer_ip IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("Peer", "label")], [("Events", "value")]),
        ("Routing Instances", "h-bar", f"SELECT juniper_routing_instance AS label, count(*) AS value FROM {s} WHERE juniper_routing_instance IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("Instance / VRF", "label")], [("Events", "value")]),
    ...
    access = [
        ...
        ("Top Administrative Users", "h-bar", f"SELECT juniper_username AS label, count(*) AS value FROM {s} WHERE juniper_username IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("User", "label")], [("Events", "value")]),
    ...
    security = [
        ...
        ("Top SRX Policies", "h-bar", f"SELECT juniper_policy AS label, count(*) AS value FROM {s} WHERE juniper_policy IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("Policy", "label")], [("Events", "value")]),
        ("Source to Destination Zones", "h-bar", f"SELECT concat(coalesce(juniper_source_zone,'unknown'),' -> ',coalesce(juniper_destination_zone,'unknown')) AS label, count(*) AS value FROM {s} WHERE juniper_source_zone IS NOT NULL OR juniper_destination_zone IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("Zone Flow", "label")], [("Events", "value")]),
...
def proxmox_ve_dashboard() -> dict:
    ...
    trends = [
        ("Events Over Time", "line", f"SELECT histogram(_timestamp, '1 hour') AS ts, count(*) AS value FROM {s} GROUP BY ts ORDER BY ts", [("Time", "ts")], [("Events", "value")]),
        ("Severity Distribution", "donut", f"SELECT coalesce(syslog_severity,'unknown') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC", [("Severity", "label")], [("Events", "value")]),
        ("Events by Node", "h-bar", f"SELECT coalesce(device_name,host_name,source_ip,'unknown') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC LIMIT 20", [("Node", "label")], [("Events", "value")]),
        ("Top Services", "h-bar", f"SELECT coalesce(syslog_program,'unknown') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC LIMIT 25", [("Service", "label")], [("Events", "value")]),
    ]
    authentication = [
        ...
        ("Top Users", "h-bar", f"SELECT proxmox_user AS label, count(*) AS value FROM {s} WHERE proxmox_user IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("User", "label")], [("Events", "value")]),
        ("Top Login Source IPs", "h-bar", f"SELECT proxmox_source_ip AS label, count(*) AS value FROM {s} WHERE proxmox_source_ip IS NOT NULL AND lower(syslog_program) IN ('sshd','pvedaemon','pveproxy') GROUP BY label ORDER BY value DESC LIMIT 25", [("Source IP", "label")], [("Events", "value")]),
    ...
    guests = [
        ("Top VM IDs", "h-bar", f"SELECT proxmox_vmid AS label, count(*) AS value FROM {s} WHERE proxmox_vmid IS NOT NULL AND proxmox_vmid <> '' GROUP BY label ORDER BY value DESC LIMIT 25", [("VM ID", "label")], [("Events", "value")]),
        ("Top Container IDs", "h-bar", f"SELECT proxmox_ctid AS label, count(*) AS value FROM {s} WHERE proxmox_ctid IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("CT ID", "label")], [("Events", "value")]),
    ...
    tasks = [
        ("Task Types", "h-bar", f"SELECT coalesce(proxmox_task,'unknown') AS label, count(*) AS value FROM {s} WHERE proxmox_upid IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("Task", "label")], [("Events", "value")]),
        ("Tasks by User", "h-bar", f"SELECT coalesce(proxmox_user,'unknown') AS label, count(*) AS value FROM {s} WHERE proxmox_upid IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 25", [("User", "label")], [("Tasks", "value")]),
...
def ubersmith_dashboard() -> dict:
    ...
    trends = [
        ("Events Over Time", "line", f"SELECT histogram(_timestamp, '1 hour') AS ts, count(*) AS value FROM {s} GROUP BY ts ORDER BY ts", [("Time", "ts")], [("Events", "value")]),
        ("Severity Distribution", "donut", f"SELECT coalesce(syslog_severity,'unknown') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC", [("Severity", "label")], [("Events", "value")]),
        ("Events by Program", "h-bar", f"SELECT coalesce(syslog_program,'unknown') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC LIMIT 25", [("Program", "label")], [("Events", "value")]),
        ("Events by Host", "h-bar", f"SELECT coalesce(device_name,host_name,source_ip,'unknown') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC LIMIT 20", [("Host", "label")], [("Events", "value")]),
    ]
    errors = [
        ...
        ("Errors by Program", "h-bar", f"SELECT coalesce(syslog_program,'unknown') AS label, count(*) AS value FROM {s} WHERE lower(coalesce(syslog_severity,'')) IN ('emerg','alert','crit','err','emergency','critical','error') GROUP BY label ORDER BY value DESC LIMIT 25", [("Program", "label")], [("Errors", "value")]),
    ...
    services = [
        ...
        ("Events by Source IP", "h-bar", f"SELECT source_ip AS label, count(*) AS value FROM {s} WHERE source_ip IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 20", [("Source IP", "label")], [("Events", "value")]),
...
def unclassified_dashboard() -> dict:
    ...
    trends = [
        ...
        ("Facility Distribution", "h-bar", f"SELECT coalesce(syslog_facility,'unknown') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC LIMIT 25", [("Facility", "label")], [("Events", "value")]),
    ]
    sources = [
        ("Top Unknown Source IPs", "h-bar", f"SELECT source_ip AS label, count(*) AS value FROM {s} WHERE source_ip IS NOT NULL GROUP BY label ORDER BY value DESC LIMIT 30", [("Source IP", "label")], [("Events", "value")]),
        ("Top Reported Hostnames", "h-bar", f"SELECT coalesce(host_name,'unknown') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC LIMIT 30", [("Hostname", "label")], [("Events", "value")]),
    ...
    programs = [
        ("Top Programs", "h-bar", f"SELECT coalesce(syslog_program,'unknown') AS label, count(*) AS value FROM {s} GROUP BY label ORDER BY value DESC LIMIT 30", [("Program", "label")], [("Events", "value")]),
    ...
    errors = [
        ...
        ("Errors by Unknown Source", "h-bar", f"SELECT coalesce(source_ip,'unknown') AS label, count(*) AS value FROM {s} WHERE lower(coalesce(syslog_severity,'')) IN ('emerg','alert','crit','err','emergency','critical','error') GROUP BY label ORDER BY value DESC LIMIT 30", [("Source IP", "label")], [("Events", "value")]),
    ...
    records = {
        ...
        "pmg": {
            **common,
            ...
            "mail_header_sender": "header@example.invalid",
