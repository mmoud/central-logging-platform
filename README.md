...
Run `./scripts/provision-dashboards.py` to idempotently create or update the bundled **Central Logging Overview**, **PMG Mail Reporting**, **PMG Message Investigation**, **PMG to Ubersmith Mail Ingestion**, **FortiGate Security & Traffic**, **Juniper Router Operations**, **Proxmox VE Operations**, **Ubersmith Billing Operations**, and **Unclassified Source Discovery** dashboards through OpenObserve's supported API. Each vendor remains isolated by dashboard and stream. PMG reporting summaries are separate from investigation; the PMG investigation and Ubersmith ingestion dashboards expose sender and recipient text inputs. The Ubersmith ingestion view correlates RFC Message-ID and Postfix queues but does not claim ticket creation until Ubersmith emits a shared mail/ticket identifier. The vendor dashboards have a query-backed Device selector (Source IP for unclassified logs); mail dashboards default to seven days and the operational dashboards to 24 hours. Dashboards are grouped into purpose-specific GUI folders.
